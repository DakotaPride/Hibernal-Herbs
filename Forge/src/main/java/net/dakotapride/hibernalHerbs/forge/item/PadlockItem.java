...
public class PadlockItem extends Item implements ITooltipProvider, ICurioItem {
    public Sins sin;

    public PadlockItem(Sins sin, Item.Settings settings) {
        super(settings);
        this.sin = sin;
    }
    ...
    public void curioTick(SlotContext slotContext, ItemStack stack) {
        ...
        switch (sin) {
            //case WRATH -> {}
            case PRIDE -> {
                slotContext.entity().addStatusEffect(new StatusEffectInstance(StatusEffects.REGENERATION, 40, 1));
            }
            case LUST -> {
                if (slotContext.entity().hasStatusEffect(StatusEffects.POISON)) {
                    slotContext.entity().removeStatusEffect(StatusEffects.POISON);
                } else if (slotContext.entity().hasStatusEffect(StatusEffects.WITHER)) {
                    slotContext.entity().removeStatusEffect(StatusEffects.WITHER);
                }
            }
            case GLUTTONY -> {
                slotContext.entity().removeStatusEffect(StatusEffects.HUNGER);
            }
            case GREED -> {
                slotContext.entity().addStatusEffect(new StatusEffectInstance(StatusEffects.LUCK, 40, 1));
            }
            //case ENVY -> {}
            case SLOTH -> {
                slotContext.entity().addStatusEffect(new StatusEffectInstance(StatusEffects.SLOW_FALLING, 40, 0));
            }
        }
    ...
    public Multimap<EntityAttribute, EntityAttributeModifier> getAttributeModifiers(SlotContext slotContext, UUID uuid, ItemStack stack) {
        ...
        switch (sin) {
            case WRATH -> {
                atts.put(EntityAttributes.GENERIC_MAX_HEALTH, wrath_health);
                atts.put(EntityAttributes.GENERIC_ATTACK_DAMAGE, wrath_damage);
            }
            case PRIDE -> {
                atts.put(EntityAttributes.GENERIC_MAX_HEALTH, pride_health);
            }
            case LUST -> {
                atts.put(EntityAttributes.GENERIC_MAX_HEALTH, lust_health);
            }
            case GLUTTONY -> {
                atts.put(EntityAttributes.GENERIC_MAX_HEALTH, gluttony_health);
            }
            case GREED -> {
                atts.put(EntityAttributes.GENERIC_MAX_HEALTH, greed_health);
            }
            case ENVY -> {
                atts.put(EntityAttributes.GENERIC_MAX_HEALTH, envy_health);
                atts.put(EntityAttributes.GENERIC_MOVEMENT_SPEED, envy_movement);
            }
            case SLOTH -> {
                atts.put(EntityAttributes.GENERIC_MAX_HEALTH, sloth_health);
                atts.put(EntityAttributes.GENERIC_MOVEMENT_SPEED, sloth_movement);
            }
        }
    ...
    public void onEquip(SlotContext slotContext, ItemStack prevStack, ItemStack stack) {
        if (slotContext.entity() instanceof ServerPlayerEntity serverPlayer) {
