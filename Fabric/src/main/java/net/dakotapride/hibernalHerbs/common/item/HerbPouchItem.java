...
public class HerbPouchItem extends Item {
    ...
    public HerbPouchItem(Item.Settings settings) {
        super(settings);
    }

    public static float getAmountFilled(ItemStack stack) {
        return getBundleOccupancy(stack) / 64.0F;
    }
    ...
    public boolean onStackClicked(ItemStack stack, Slot slot, ClickType clickType, PlayerEntity player) {
        ...
        if (itemStack.isEmpty()) {
            ...
            removeFirstStack(stack).ifPresent(removedStack -> addToBundle(itemStack, slot.insertStack(removedStack)));
    ...
    private static Optional<NbtCompound> canMergeStack(ItemStack stack, NbtList items) {
        ...
        Objects.requireNonNull(NbtCompound.class); return items.stream().filter(NbtCompound.class::isInstance).map(NbtCompound.class::cast)
    ...
    private static Stream<ItemStack> getBundledStacks(ItemStack stack) {
        ...
        NbtList nbtList = nbtCompound.getList("Items", 10);
        Objects.requireNonNull(NbtCompound.class); return nbtList.stream().map(NbtCompound.class::cast).map(ItemStack::fromNbt);
    }


    public Optional<TooltipData> getTooltipData(ItemStack stack) {
        DefaultedList<ItemStack> defaultedList = DefaultedList.of();
        Objects.requireNonNull(defaultedList); getBundledStacks(stack).forEach(defaultedList::add);
        return Optional.of(new BundleTooltipData(defaultedList, getBundleOccupancy(stack)));
    }


    public void appendTooltip(ItemStack stack, World world, List<Text> tooltip, TooltipContext context) {
        tooltip.add(Text.translatable("item.minecraft.bundle.fullness", new Object[] { Integer.valueOf(getBundleOccupancy(stack)), Integer.valueOf(64) }).formatted(Formatting.GRAY));
    }
