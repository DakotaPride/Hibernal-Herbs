package net.dakotapride.hibernalherbs.item.canister;
...
public class AbstractCanisterItem extends BundleItem implements ITooltipProvider {
    ...
    public AbstractCanisterItem(Settings settings, int size) {
        super(settings);
        this.size = size;
    }
    ...
    public boolean onStackClicked(ItemStack stack, Slot slot, ClickType clickType, PlayerEntity player) {
        if (clickType != ClickType.RIGHT) {
            ...
            } else if (itemStack.isIn(Utilities.BLENDS_ITEM_TAG)) {
    ...
    public TypedActionResult<ItemStack> use( World world, PlayerEntity player, Hand hand) {
        ...
        if (dropAllBundledItems(var4, player)) {
            ...
            player.incrementStat(Stats.USED.getOrCreateStat(ItemRegistry.CANISTER));
    ...
    public static int add(ItemStack bundleStack, ItemStack addStack, int size, PlayerEntity player) {
        if (!addStack.isEmpty() && addStack.isIn(Utilities.BLENDS_ITEM_TAG)) {
    ...
    public static List<NbtCompound> getMatchingItem(ItemStack itemStack, NbtList listTag) {
        if (itemStack.isIn(Utilities.CANISTERS_ITEM_TAG)) {
    ...
    public static int getWeight(ItemStack stack, int size) {
        if (stack.isIn(Utilities.CANISTERS_ITEM_TAG)) {
