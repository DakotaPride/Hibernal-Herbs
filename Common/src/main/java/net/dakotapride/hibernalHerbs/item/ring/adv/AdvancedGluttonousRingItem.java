package net.dakotapride.hibernalherbs.item.ring.adv;
...
public class AdvancedGluttonousRingItem extends Item implements ITooltipProvider {
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if (!Screen.hasShiftDown()) {
            ...
            tooltip.add(Text.literal(""));
            tooltip.add(Text.translatable("text.hibernalherbs.warning.gluttony.one", ItemRegistry.GLUTTONOUS_RING.getDefaultStack().getName()).formatted(Formatting.DARK_PURPLE));
        }
    }
}
